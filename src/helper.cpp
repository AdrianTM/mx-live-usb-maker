...
[[nodiscard]] const QHash<QString, QStringList> &allowedCommands()
{
    static const QHash<QString, QStringList> commands {
        ...
        {"mx-live-usb-maker-backend",
         {"/usr/lib/mx-live-usb-maker/mx-live-usb-maker-backend", "/usr/bin/mx-live-usb-maker-backend"}},
...
[[nodiscard]] ProcessResult runProcess(const QString &programPath, const QStringList &args)
{
    ...
    process.start(programPath, args, QIODevice::ReadOnly);
    if (!process.waitForStarted()) {
        printError(QString("Failed to start %1").arg(programPath));
        result.exitCode = 127;
        return result;
    }
